...
class Solver_Noise(object):
    def __init__(self, args, batch_size=64, source='renji',
                 target='luodian', learning_rate=0.0002, interval=100, optimizer='adam'
                 , num_k=4, all_use=False, checkpoint_dir=None, save_epoch=10, datafolder = '/root/autodl-nas/', ys =[],yt=[]):
        ...
        self.source_dataset, _ = get_biodata(source, self.batch_size, datafolder=self.datafolder)
        self.target_dataset, _ = get_biodata(target, self.batch_size, datafolder=self.datafolder)
        print('load finished!')
    ...
    def set_optimizer(self, which_opt='momentum', lr=0.001, momentum=0.9):
        if which_opt == 'momentum':
            self.opt_g = optim.SGD(self.G.parameters(),
                                   ...
                                   momentum=momentum)
            self.opt_f1 = optim.SGD(self.G1.parameters(),
                                   lr=lr, weight_decay=0.0005,
                                   momentum=momentum)
            self.opt_f2 = optim.SGD(self.G2.parameters(),
                                   lr=lr, weight_decay=0.0005,
                                   momentum=momentum)
            self.opt_f3 = optim.SGD(self.G2.parameters(),
                                   lr=lr, weight_decay=0.0005,
                                   momentum=momentum)
        ...
        if which_opt == 'adam':
            self.opt_g = optim.Adam(self.G.parameters(),
                                    lr=lr, weight_decay=0.0005)
            self.opt_f1 = optim.Adam(self.F1.parameters(),
                                    lr=lr, weight_decay=0.0005)
            self.opt_f2 = optim.Adam(self.F2.parameters(),
                                    lr=lr, weight_decay=0.0005)
            self.opt_f3 = optim.Adam(self.F2.parameters(),
                                    lr=lr, weight_decay=0.0005)
    ...
    def reset_grad(self):
        ...
        self.opt_f1.zero_grad()
        self.opt_f2.zero_grad()
        self.opt_f3.zero_grad()
    ...
    def train(self, epoch, record_file=None):
        ...
        self.datazip = enumerate(zip(self.source_dataset,self.target_dataset))

        if self.target == 'huashan':
            datanum = 310
        elif self.target == 'renji':
            datanum = 759
        if self.target == 'luodian':
            datanum = 604  
        if self.target == 'luodian_':
            datanum = 598

        if self.source  == 'huashan':
            datanum_s = 310
        if self.source == 'renji':
            datanum_s = 759
        if self.source == 'luodian':
            datanum_s = 604
            
        new_ys = np.zeros([datanum_s, 2])#source
        new_yt = np.zeros([datanum, 2])#2 means classnum
        #前面是源域数据，后面是目标域数据，没有标签
        for batch_idx, ((img_s, label_s, index_s), (img_t, _, index_t)) in self.datazip:
            ...
            #step A: 使用源域数据来训练一个初始模型，同时初始化ys_tilde(new_ys)（初始化标签），对源域数据的全监督。
            if epoch < 1:
                ...
                if batch_idx % self.interval == 0:
                    print(self.source+'--Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss1: {:.6f}\t Loss2: {:.6f}'.format(
                    epoch, batch_idx, 100,
            ...
            #Step B：
                #a：最大化分类器差异的同时，减小源域数据的兼容性损失（每一次epoch都会有一个新的预测，和之前的最开始的初始化标签），最小化源域和目标域的KL散度（改良版）。
                #b：最小化分类器差异
            elif epoch < 105 and epoch >= 1:
                #part a
                feat_t = self.G(img_t)
                feat_s1 = self.F1(feat_s)
                feat_s2 = self.F2(feat_s1)
                #feat_s3 = self.F3(feat_s2)

                feat_t1 = self.F1(feat_t)
                feat_t2 = self.F2(feat_t1)
                #feat_t3 = self.F3(feat_t2)

                output_s1 = self.C1(feat_s1)
                output_s2 = self.C2(feat_s1)
                
                output_t1 = self.C1(feat_t1)
                output_t2 = self.C2(feat_t1)
                ...
                #计算损失
                loss_mmd1 = mmd_loss(feat_s1, feat_t1)
                loss_mmd2 = mmd_loss(feat_s2, feat_t2)
                #loss_mmd3 = mmd_loss(feat_s3, feat_t3)
                #loss_mmd = loss_mmd1+loss_mmd2+loss_mmd3
                loss_mmd = loss_mmd1+loss_mmd2
                ...
                '''discrepancy'''#计算双分类器对于目标域数据的输出的距离损失函数
                ...
                for i in range(self.num_k):
                    ...
                    output_tc1 = self.C1(feat_t2)
                    output_tc2 = self.C2(feat_t2)

                    '''discrepancy'''#对抗，前面是越大越好，这里是越小越好。
                    loss_dis = self.discrepancy(output_tc1, output_tc2)
                ...
                if batch_idx % self.interval == 0:
                    print('Source:'+self.source+' Target:'+self.target+'Train Epoch: {} [{}/{} ({:.0f}%)]\tLossMmd: {:.6f}\t LossCo: {:.6f}\t  Discrepancy: {:.6f}'.format(
                    epoch, batch_idx, 100,
                    100. * batch_idx / 70000, loss_mmd.data, lo_s.data, loss_dis.data))
                    if record_file:
                        record = open(record_file, 'a')
                        record.write('%s %s %s\n' % (loss_dis.data, loss_mmd.data, lo_s.data))
                        record.close()
            #step C:使用交叉熵进行最后的微调，使用上一轮最后确定下来的标签来当作真实标签。
            else:
                feat_t = self.G(img_t)
        ...
        if epoch < 1:
            # save y_tilde
            ys = new_ys
            ys_file = datafolder + "ys.npy"#上一轮的标签保存在这里，调用的时候使用idx索引。
            np.save(ys_file,ys)
    ...
    def test(self, epoch, record_file=None, save_model=False):
        ...
        if epoch < 1:
            self.dataset_test = enumerate(zip(self.source_dataset,self.target_dataset))
            ...
            for batch_idx, ((_, __, ___), (img, label, _)) in self.dataset_test:
                ...
                metric_collection.forward(output_ensemble,label)
                '''pred1 = output1.data.max(1)[1]
                pred2 = output2.data.max(1)[1]
                pred_ensemble = output_ensemble.data.max(1)[1]
                pred_s1.append(pred1.cpu())
                pred_s2.append(pred1.cpu())
                pred_s3.append(pred1.cpu())
                label_s.append(label.data.cpu())
            ...
            print(
                '\nTest set: Best Average loss: {:.4f},  Best  Accuracy: ({:.4f}%)  Best Precision: ({:.4f}%) Best  Recall:  ({:.4f}%)\n'.format(
                    test_loss, 100*self.acc, 100*self.prec, 100*self.rec))
